...
class LoginViewController : UIViewController, ValidationDelegate {
    ...
    let userInterface : UserInterface = UserInterface()
    ...
    override func viewDidLoad() {
        ...
        // check internet connection as the initial first screen
        checkConnection()
    ...
    private func checkConnection() {
        BaseInterface.testConnection()
    }
    
    // This is to reset the error indicator by the validator
        ...
        userInterface.loginUser(userInfo: userInfo).continueWith { task in
            if task.cancelled {
                ...
                if let code = json?["code"].number {
                    switch code {
                    case 1001:
                        showError(message: "You are not registered.\nSign up now.")
                    case 422 :
    ...
    // Highlight and show the error for failed validation
